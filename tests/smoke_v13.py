"""Smoke tests sans appels réseau externes pour SKYLINE AIRWAYS v1.3."""
...
r=c.get('/health'); check(r.status_code==200,'health'); check(r.json()['version']=='1.3.0','version')
...
r=c.get('/game'); check(r.status_code==200,'game'); check('game-v130.js?v=130' in r.text,'v130 JS'); check('premium-v130.css?v=130' in r.text,'v130 CSS')
...
# NOTAM sans secret doit être propre, jamais simulé.
n=c.get('/api/aviation/notam/status').json(); check(n['configured'] is False,'NOTAM secret state')
